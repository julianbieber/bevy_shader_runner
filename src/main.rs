...
struct Opt {
    ...
    #[arg(
        long,
        help = "Dump shader with hardcoded slider values for Shadertoy. Use '-' for stdout. Will watch for slider changes when running interactively"
    )]
...
fn setup_dump_watcher(shader_path: PathBuf, dump_path: String) -> DumpConfig {
    // Initial dump
...
fn check_for_changes_and_dump(slider_state: Res<SliderState>, dump_config: Res<DumpConfig>) {
    ...
    if slider_changed {
        if let Ok(elapsed) = now.duration_since(dump_config.last_dump_time) {
            if elapsed >= debounce_duration {
                let config =
                    read_slider_state_for_dump(&dump_config.shader_path).unwrap_or_default();
                if let Err(e) =
                    dump_shadertoy_shader(&dump_config.shader_path, &config, &dump_config.dump_path)
                {
...
pub fn convert_to_shadertoy(
    ...
    lines.retain(|line| {
        !line.trim().starts_with("#version")
            ...
            && !line
                .trim()
                .starts_with("layout(location = 0) out vec4 fragColor;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 0) uniform float iTime;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 1) uniform vec2 iResolution;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 2) uniform vec4 slider1;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 3) uniform vec4 slider2;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 4) uniform vec4 slider3;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 5) uniform vec4 slider4;")
            && !line
                .trim()
                .starts_with("layout(set = 2, binding = 6) uniform vec4 slider5;")
    });
